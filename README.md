...
#### Анализ уязвимостей
  ...
  Ценность актива: метрики производительности, системная информация
  
  Сложность эксплуатации: Low (публично доступно)
  
  Ущерб: раскрытие системной информации, помощь в дальнейшей атаке
  
  2. Missing Security Headers (8 шт)
  
  Тип: Истинная уязвимость
  
  Критичность: Low-Medium (CVSS ~3.0-4.0)
  
  Доступность: интернет
  
  Ценность актива: веб-приложение, пользовательские данные
  
  Сложность эксплуатации: Medium (требует доп. атак)
  
  Ущерб: XSS, clickjacking, снижение безопасности браузера
  
  Приоритеты устранения: Prometheus Metrics - раскрытие системной информации
...
#### Меры для найденных уязвимостей

  1) Prometheus Metrics:
  
  Краткосрочно: блокировать /metrics в nginx
  
  Долгосрочно: аутентификация для метрик
  
  2) Security Headers:
  
  Краткосрочно: добавить заголовки в веб-сервер
  
  Долгосрочно: встроить в приложение

## Ответы на контрльные вопросы
...
- SAST лучше находит SQL-инъекции, XSS, утечки памяти, небезопасные криптографические функции, hardcoded секреты.
...
- Документирование низкорисковых уязвимостей необходимо для аудита, отслеживания технического долга, обоснования решений перед руководством.
